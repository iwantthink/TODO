...
## 10.2 获取`mPackages`
 ...
 获取到`ActivityThread`的`ArrayMap<String, WeakReference<LoadedApk>> mPackages`,需要拿到这个集合中对应当前APP的`LoadedApk`
...
## 10.3 获取`PathClassLoader`
 拿到`LoadedApk`中的`PathClassLoader`,这个是系统类加载器.同一个包对应的类加载器都是同一个.
...
## 10.4 获取`DexPathList`
拿到`PathClassLoader`中的`DexPathList`,前者主要依靠后者去实现类加载.
...
- **这里需要注意:`pathList`字段是存在于`BaseDexClassLoader`,所以反射需要使用的是`BaseDexClassLoader`的字节码**

## 10.5 获取`Element[]`

拿到`DexPathList`中的`Element[]`,`Element`就是代表的含有`.dex`文件的信息
...
手动生成Element 插件 有俩种方式

1. 直接调用`Element`的构造函数,传入插件的信息等参数 创建出对象

2. 通过`DexPathClassLoader`将插件加载进来,然后通过反射获取到`DexClasLoader`对象中的`Element[]`


### 10.6.1 通过构造函数

	//获取`Element[]`数组的 类型的字节码
	Class<?> element_Class = old_dexElements.getClass().getComponentType();
	//获取`Element`构造函数
	Constructor element_constructor =
                    element_Class.getDeclaredConstructor(File.class,                           boolean.class,                            File.class,                           DexFile.class);
	element_constructor.setAccessible(true);
	//构建参数
	//创建保存opt的地址
	File optDexFile = getFileStreamPath("debug.dex");
	Object newElement = element_constructor.newInstance(
					apkFile,
					false,
                    apkFile,
                    DexFile.loadDex(
     ...
     Object[] new_dexElements = new Object[]{newElement};

- 创建出来的是`Element`对象


### 10.6.2 通过DexPathClassLoader
...
- 首先需要创建`DexPathClassLoader`将插件加载到内存,然后通过反射获取到自定义类加载器中的`DexPathList`,最后同样通过反射获取到`DexPathList`中的`Element[]`数组

- **注意:`new_dexElements`是数组类型,可以换成如下形式:**

		Object [] new_dexElements =(Object []) dexElements_field.get(new_pathList);
...
资源加载上仍然有问题,当加载了一个插件中的Activity时,当插件Activity加载layout时,会使用到不正确的资源id,导致加载错误的布局

**暂时的解决方式是:布局全部通过代码编写 .仍然需要寻求插件系统的资源管理机制**

# 11. 总结

**[方案一]:**手动创建了一个解析了插件信息的`LoadedApk`并放到了`ActivityThread`中的`ArrayMap mPackages`中,同时会在开启插件Activity时 将Intent 中的 pkg-name 信息改成 插件的包名(为了命中`mPackages`中的`LoadedApk`).

- 方案一 采用的多`ClassLoader`架构,即插件和宿主的类加载器是分开的,同时每个插件都有各自的类加载器.

- 多`ClassLoader`可以完成真正的代码热加载,如果插件更新,直接重新创建一个自定义类加载器去加载插件,然后替换掉原来版本即可(这是因为 同一个类 被不同类加载器加载 也算是不同的类)

**[方案二]:**Hook掉了系统类加载器`PathClassLoader`,获取到其`Element[]`数组,然后手动组装了一个代表插件(包含有`.dex`文件即可)信息的`Element`,并入原先的数组 并替换掉`PathClassLoader`中的`Element[]`数组

- 方案二 采用的单`ClassLoader`架构,即插件和宿主的类加载器是共享的,这样可能就会导致插件和宿主存在类冲突,资源冲突等

- 单`ClassLoader`在实现热更新时 可能需要进程重启


**选择多`ClassLoader`架构在大多数情况下是明智之举。**

**开源框架中,`DroidPlugin`采用的 方案一,`Small`采用的 方案二**
