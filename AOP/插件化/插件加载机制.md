...
1. 全面接管系统的类加载过程,添加一个加载插件`DexClassLoader`.
	...
	这种方案就需要往`mPackages`中添加能加载 插件的`LoadedApk`
...
3. **注意,`LoadedApk`是通过包名获取的,如果想要获得指定的`LoadedApk`就需要传入指定的 包名!!!!!!,这点非常重要,如果想要获取到 加载插件的LoadedApk 那就必须传入 其对应的 包名,那就要求在绕过AMS检查 替换Intent的时候 将这个包名信息 也传入 `LoadedApk`**
...
- **`mPackages`中保存的是 关系是: 包名-LoadedApk**,插件如果是`.dex`形式,那么可能无法进行Hook(没有包名信息). 

## 4.4 Hook方案

根据之前的解决思路,可以有俩套方案去Hook 这个点:

1. 针对`mPackages`缓存进行Hook,将原始的`LoadedApk.getClassLoader()`方法进行Hook,使其能返回修改过的`DexClassLoader`.这样在`perfromLaunchActivity()`中即可找到插件PKG对应的`ClassLoader`所在,并执行接下来的逻辑.
...
2. 直接构建LoadedApk对象,**这里需要注意,直接创建的`LoadedApk`缺少一些参数,我们需要手动替他添加**
...
上面的代码,在Android 22 上是能够正确运行.但是在Android 24 上是会抛出一个异常
