...
总来来说分为三大类分别是：`Application Window`（应用程序窗口）、`Sub Windwow`（子窗口）、`System Window`（系统窗口），每个大类又包含了很多种类型，它们都定义在`WindowManager`的静态内部类`LayoutParams`中
