...
- taskX 对 taskY 的依赖在 taskY 被定义之前就已经声明了.

**当所关联的任务还没有被定义,不能使用快捷注释**


# 5. 动态任务

Groovy 不仅可以被用来定义一个任务可以做什么，还可以使用它来动态的创建任务等操作
...
- doFirst 和 doLast 可以被执行许多次. 它们的作用是分别在任务动作列表的开始和结束加入动作. 当任务执行的时候, 在动作列表里的动作将被按顺序执行.



# 7. 短标记法

**短标记 `$` 可以用来访问一个当前存在的任务. 也就是说每个任务都可以作为构建脚本的属性**
...
- 例如加入一个叫做 myProperty 属性, 设置一个初始值给 ext.myProperty. 然后, 该属性就可以像一个预定义的任务属性那样被读取和设置了.
...
**从构建的生命周期可知，Gradle 有一个配置阶段和执行阶段. 在配置阶段后, Gradle 将会知道应执行的所有任务. 此外Gradle 为你提供一个"hook", 以便利用这些信息**

- 举个例子, 判断发布的任务是否在要被执行的任务当中. 根据这一点, 你可以给一些变量指定不同的值.
