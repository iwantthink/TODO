...
带引号的标识符出现在点表达式的点(`.`)之后。例如，`person.name`表达式的`name`部分可以与加引号(`person."name"`或`person.'name'`)

- 当某些标识符包含Java语言规范所禁止的非法字符，但Groovy却允许在引用时使用这些字符，这一点特别有用。例如，字符如破折号(`-`)、空格、感叹号等
...
Groovy提供了不同的字符串文本,并且所有类型的字符串都允许在点之后使用
...
普通字符串和Groovy的`GStrings`（插值字符串）之间存在差异，因为在`Gstrings`中，插值将插入最终字符串中以评估整个标识符
...
除了单引号字符串和三引号字符串外，任何Groovy表达式都可以插入到所有字符串文本中。插值是在字符串中的占位符，在估值时用其值替换字符串中的占位符。占位符表达式被`${}`包围，或者用`$`作为表达式前缀

- 当`GString`被传递给以`String`为参数的方法时，会通过调用该表达式上的`toString()`方法将包含占位符的表达式值计算为字符串表示形式
...
- 任何Groovy表达式都是有效的，例如算术表达式：

		def sum = "The sum of 2 and 3 equals ${2 + 3}"
		assert sum.toString() == 'The sum of 2 and 3 equals 5'
	
**`$ {}`占位符之间不仅允许表达式，还允许语句**。但是因为语句的值是`null`，所以如果在该占位符中插入了多个语句，则最后一个语句应返回一个有意义的值以便插入。 例如，`${def a = 1; def b = 2; a + b}`用来表示1+2的和,但是**建议在`GString`的占位符中使用简单表达式**
...
- **只有`a.b`，`a.b.c`等形式的点表达式才有效，包含括号的表达式（如方法调用，闭包的花括号或算术运算符）将无效**
...
到目前为止，已知可以在`${}`占位符中插入任意表达式，但是对于闭包表达式有一个特殊的情况和符号。当占位符包含一个箭头`${->}`时，表达式实际上是一个闭包表达式，可以将它看作一个前面带`$`符号的闭包
...
美元斜线字符串是用开头`$/`结尾`/$`为分隔的多行`GStrings`。转义字符是`$`符号，它可以转义`$`或正斜杠(`/`)。但是`$`和正斜杠默认都不需要转义，除非 出现类似`GString`占位符序列的不字符串(不使用占位功能，仅输出)，或者出现类似美元斜线字符串结尾的符号`/$`
...
为了便于进行精确的小数计算，Groovy选择`java.lang.BigDecimal`作为默认的小数类型。 此外，Groovy同样支持`float`和`double`，但是使用这俩种类型需要手动指定类型，强制转换，或者使用后缀
...
当操作数是`float`或`double`，则除法运算符`/`（和/ =用于除法和赋值)计算的结果是`double`
...
# 7. 列表
Groovy使用逗号分隔开的值列表(由方括号`[]`包围)来表示列表。因为Groovy没有定义自己的集合类,所以Groovy中的列表就是JDK的`java.util.List`。定义列表时使用的具体列表实现是`java.util。ArrayList`，这是默认值
...
**Groovy为数组重用了列表表示法，但是为了创建数组，还需要通过强制或类型声明来明确地定义数组的类型**
...
在其他语言中，`Map`有时被称为字典或关联数组。`Maps`的作用是将键与值相关联，其使用冒号(`:`)分隔键和值，使用逗号(`,`)分隔每个键/值对，使用方括号`[]`包含所有键值对
...
如果想要传递一个变量来作为键，那么该变量本身会成为键，而不是该变量的值作为键
...
当需要强制使用变量作的值为`Map`的键，则可以通过 用圆括号将变量或表达式括起来来实现
