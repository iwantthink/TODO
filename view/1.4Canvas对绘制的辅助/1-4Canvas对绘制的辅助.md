...
>**前乘pre:  M` = M·S （前乘相当于矩阵的右乘）**
> 
>**后乘post:  M` = S·M (后乘相当于矩阵的左乘 )** 
>  
...
## 2.3 使用Camera做三维变换
Camera使用的是三维坐标系，x轴 左正右负  y轴 上正下负  z轴 屏幕朝里为负，朝外为正  
...
因为默认原点的关系，如果我们想实现 翻转时 对图形进对称的操作。需要移动图形到原点位置！然后再进行操作！最后再移动回去~ (另外这个移动需要使用canvas来进行，因为camera 移动的话 是移动的camera的坐标轴。。)  
